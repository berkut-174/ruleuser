...
class Program:
    ...
    def callback(self, widget, data1=None, data2=None, data3=None):
        ...
        if data1 == "screenshot":
            ...
            if os.path.exists(self.cfg.vncShotFolder):
                pix.save(self.cfg.vncShotFolder + "/" + label_text + "_" + time.strftime("%d.%m.%Y_%H:%M:%S") + ".png",
                         "png", {})
    ...
    def thumbnails_reorder(self, mode="", resize_vbox=None, data=None):
        ...
        if self.cfg.vnc_box == []:
            return
        if mode == "all_minimize" and resize_vbox is None:
                    ...
                    if ((self.cfg.vnc_box[num][13] == "custom" and mode != "all_minimize") or
                        ...
                        if mode == "all_minimize":
                            ...
                            if self.cfg.vnc_box[num][13] != "min":
                                ...
                                self.cfg.vnc_box[num][2].set_size_request(self.cfg.vnc_box[num][4],
                                                                          self.cfg.vnc_box[num][5])
    ...
    def thumbnails_vnc_event_button(self, button, p, mode):
        ...
        if mode == "screenshot":
            ...
            if p[13] == "min":
                p[13] = "max"
                (new_max_thumb_x, new_max_thumb_y, new_max_window_x, new_max_window_y) = self.vnc_thumbnails_size(
                    "max")[4:8]
    ...
    def thumbnails_vnc(self, user_list, param):
        ...
        for z in user_list:
            ...
            self.table_vnc.put(vbox, col * (int(new_thumb_x) + self.cfg.tt_border),
                               row * (new_thumb_y + self.cfg.tt_border))
    ...
    def table_vnc_motion_event(self, widget, event):
        ...
        if p and p[2] and p[25] and p[26]:
            ...
            p[4] = thumb_x
            p[5] = thumb_y
            self.table_vnc.move(p[26], vbox_alloc.x + thumb_x - int(self.cfg.tt_border * 0.5) - 9,
                                vbox_alloc.y + thumb_y - int(self.cfg.tt_border * 0.5) - 9)
            self.thumbnails_reorder("all")
    ...
    def thumbnails_vnc_pre(self, user_list, mode):
        ...
        (window_x, window_y, real_x, real_y, new_min_thumb_x, new_min_thumb_y, new_min_window_x,
         new_min_window_y) = self.vnc_thumbnails_size("min")
        (window_x, window_y, real_x, real_y, new_max_thumb_x, new_max_thumb_y, new_max_window_x,
         new_max_window_y) = self.vnc_thumbnails_size("max")
            ...
            for x in user_list:
                ...
                if rep == False:
                    ...
                    self.cfg.vnc_box.insert(item,
                                            ["empty", "", "", "", new_thumb_x,
                                             new_thumb_y, new_min_thumb_x, new_min_thumb_y, new_max_thumb_x,
                                             new_max_thumb_y,
                                             None, None, None, None, None,
                                             None, None, None, None, None,
                                             None, None, None, None, None,
    ...
    def mainUi(self, data=None):
        ...
        self.itemVncMinimize = menu_image_button(self.cfg.pixbuf_action_window_min_16,
                                                 _("Thumbnails") + " - " + _("Minimize all") + "\tF3")
        self.itemVncMinimize.connect("activate", self.view_vnc_minimize)
        self.menuView.append(self.itemVncMinimize)

        self.itemVncClose = menu_image_button(self.cfg.pixbuf_action_close_16,
                                              _("Thumbnails") + " - " + _("Close all") + "\t\t\tF4")
        ...
        if drag_context.get_source_widget() == self.cfg.treeView:
            ...
            for p in self.cfg.vnc_box:
                ...
                if alloc.x < x < alloc.x + alloc.width + self.cfg.tt_border and alloc.y < y < alloc.y + alloc.height:
                    ...
                    break
            if p_source and p_target_index is not None and p_target_index != p_source_index:
