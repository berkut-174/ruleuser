...
def create_tree_server_group(cfg, server_iter, z_server=None, alias_list=None):
    ...
    if z_server[0] not in cfg.localhost and validate("ip", z_server[0]) == False:
        ...
        return
    if (z_server[0] not in cfg.localhost and nmap_socket(cfg, z_server[0], "22", z_server[cfg.dn['timeout']], None,
                                                         name=None) == "0"):
    ...
    if ds['demo_vlc_client_command'] == "":
        ds[
            'demo_vlc_client_command'] = "vlc --network-caching=100 --qt-minimal-view --no-qt-privacy-ask --no-qt-error-dialogs"
    ...
    if ds['alias'] in cfg.localhost:
        # user, host, display
        clients = get_user_env(cfg, "clients", None, ds['server_key'], ds['server_port'], ds['server_user'],
                               ds['alias'], name=name)
        # ip, mac
        arp = get_user_env(cfg, "arp", None, ds['server_key'], ds['server_port'], ds['server_user'], ds['alias'],
                           name=name)
        # desktop
        desktops = get_user_env(cfg, "desktops", None, ds['server_key'], ds['server_port'], ds['server_user'],
                                ds['alias'], name=name)
        # passwd
        # user:x:uid:gid:gecos:home:shell
        passwd = get_user_env(cfg, "passwd", None, ds['server_key'], ds['server_port'], ds['server_user'], ds['alias'],
                              name=name)
    else:
        (clients, arp, desktops, passwd) = get_user_env(cfg, "all", None, ds['server_key'], ds['server_port'],
                                                        ds['server_user'], ds['alias'], name=name)
    ...
    for x in range(int(cfg.groups)):
        ...
        for y in range(int(hosts)):
            ...
            if cfg.userList.get_value(iter, cfg.dn['demo_vlc_client_command']) == "":
                cfg.userList.set(iter, cfg.dn['demo_vlc_client_command'],
                                 "vlc --network-caching=100 --qt-minimal-view --no-qt-privacy-ask --no-qt-error-dialogs")
            if not "--no-qt-privacy-ask" in cfg.userList.get_value(iter, cfg.dn['demo_vlc_client_command']):
                cfg.userList.set(iter, cfg.dn['demo_vlc_client_command'], cfg.userList.get_value(iter, cfg.dn[
                    'demo_vlc_client_command']) + " --no-qt-privacy-ask")
...
def tree_move(widget, cfg, treeView, action):
    ...
    for row_iter in temp_row_iter:
        ...
        if (new_iter and treeView != cfg.treeView and row[0] != dest_path[0] and check_demo_client_group(cfg, model,
                                                                                                         new_iter)):
...
def save_userList_start(cfg):
    ...
    for x in range(len(cfg.userList)):
        ...
        if group == "server":
            ...
            item = ""
            for part in range(len(cfg.z)):
                if cfg.userList[(x,)][part] is None:
                    cfg.userList[(x,)][part] = ""
            ...
            for y in range(cfg.userList.iter_n_children(parent_iter)):
                item = ""
                for part in range(len(cfg.z)):
                    if cfg.userList[(x, y)][part] is None:
